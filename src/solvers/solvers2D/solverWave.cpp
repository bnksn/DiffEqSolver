...
SolverWave::SolverWave(const double xFinal, const int xNumSteps, const double yFinal,
                       const int yNumSteps, const std::vector<double>& zInitial,
                       const double waveSpeed, const BoundaryCondition boundaryCondition) noexcept
...
std::vector<double> SolverWave::solve() const {
    ...
    auto zValues = std::vector<std::vector<double>>{this->zInitial, this->zInitial};
    ...
    for (auto step = 1; step < this->yNumSteps; ++step) {
        ...
        for (auto i = 1; i < prevPoints.size() - 1; ++i) {
            newPoints[i] =
                ...
                this->multiplier * (prevPoints[i + 1] - 2 * prevPoints[i] + prevPoints[i - 1]);
        }
