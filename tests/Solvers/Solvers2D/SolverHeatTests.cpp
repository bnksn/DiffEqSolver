...
TEST(SolverTests, CheckResults) {
    const auto expectedResults =
        std::vector<double>{1, 2,       2,       3,       2,       2,       1,
                            1, 1.78667, 2.21333, 2.57333, 2.21333, 1.78667, 1,
                            1, 1.70987, 2.19911, 2.41973, 2.19911, 1.70987, 1,
                            1, 1.6628,  2.14181, 2.3256,  2.14181, 1.6628,  1,
                            1, 1.62359, 2.07883, 2.24718, 2.07883, 1.62359, 1,
                            1, 1.58768, 2.01763, 2.17535, 2.01763, 1.58768, 1,
                            1, 1.55403, 1.95955, 2.10805, 1.95955, 1.55403, 1};
    ...
    const auto heatSolver =
        SolverHeat(xFinal, xNumSteps, yFinal, yNumSteps, zInitial, heatConstant,
                   boundaryCondition);
    auto results = heatSolver.solve();
